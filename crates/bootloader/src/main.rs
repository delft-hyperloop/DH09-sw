...
use embassy_boot::{BootLoader, BootLoaderConfig, Partition};
use embassy_boot_stm32::BlockingPartition;
use embassy_stm32::{
    flash::{Flash /*FlashRegion*/ },
    init, 
    Config
};
use smoltcp::iface::{InterfaceBuilder, NeighborCache, Routes, SocketSet as SmoltcpSocketSet};
...
fn main() -> ! {
    ...
    */
    ...
    let active_region = BlockingPartition::new(&flash, ACTIVE_FIRMWARE_ADDR, FIRMWARE_MAX_SIZE);
    let dfu_region = BlockingPartition::new(&flash, DFU_FIRMWARE_ADDR, FIRMWARE_MAX_SIZE);
    let state_region = BlockingPartition::new(&flash, ACTIVE_FIRMWARE_ADDR - 0x4000, 0x4000); 
    ...
    static mut NEIGH: [Option<(IpAddress, EthernetAddress)>; 8] = [None; 8];
    ...
    let mut iface = match InterfaceBuilder::new(todo!("Ethernet device (p.ETH_MAC) needs to be initialized and passed here"))
    ...
    let mut sockets = SmoltcpSocketSet::new(Vec::new());
    ...
    iface.poll(&mut sockets, SmoltcpInstant::from_millis(EmbassyInstant::now().as_millis() as i64)).ok();
    ...
    if bad_firmware {
        ...
        if let Err(_) = writer.erase(0, FIRMWARE_MAX_SIZE) {
            SCB::sys_reset(); // Reset if erase fails
        }

        'update: loop {
            if let Ok(()) = iface.poll(&mut sockets, SmoltcpInstant::from_millis(EmbassyInstant::now().as_millis() as i64)) {
            let mut s = sockets.get::<UdpSocket>(udp);
