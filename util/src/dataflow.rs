...
"#,
    ...
    writeln!(&mut code, "pub fn event_for_can_1_id(id: u32) -> fsm::commons::Event {{ match id {{")
        .unwrap();

    for (id, event) in &can1_ids_to_events {
        writeln!(&mut code, "{} => fsm::commons::Event::{},", id, event).unwrap();
    }
    ...
    writeln!(&mut code, "pub fn event_for_can_2_id(id: u32) -> fsm::commons::Event {{ match id {{")
        .unwrap();
