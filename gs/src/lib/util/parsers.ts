...
const sensorParse = (u64: bigint): number => {
    return u64 > MAX_VALUE - 100000
        ? -(MAX_VALUE - Number(u64) + 1) / 100
        : Number(u64) / 100;
};
...
const metersPerMinuteToByte = (mpm: number): number => {
    ...
    let mappedValue =
        ((mpm - speed_min) / (speed_max - speed_min)) * (byte_max - byte_min) +
        byte_min;
...
const parseShortCut = async (
    shortcut: string,
    debugMode: boolean,
    logsAreVisible: boolean
): Promise<void> => {
    ...
    if (shortcut === 'ToggleLogs') {
        ...
        if (!logsAreVisible) {
            ...
            logsScrollAreaSize.set(
                30 -
                    (30 * 0.05 + 4.5) +
                    (window.innerHeight / VIEWPORT_HEIGHT_NORMALIZING_VALUE) *
                        10 -
                    10
            );
        } else {
